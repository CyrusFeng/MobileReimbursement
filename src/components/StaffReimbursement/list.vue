<template>
    <div style="height: 100%;">
        <GoBack v-if="$store.state.UA !== 1">员工报销单列表</GoBack>
                        ...
                        <div class="wrap">
                            <ul class="content">
                                <router-link
                                        ...
                                        :key="index"
                                        :to="{name:'detailsAndApproval',params:{billId:item.id,billStatus:'SUBMIT'}}"
                                >
                                    <div class="head">
                                        <div class="left">
                            ...
                            <Loading :showloadingwrap="showLoadingWrap1" :showloadingimg="showLoadingImg1"></Loading>
                        </div>
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="list-wrap" ref="wrapper2">
                        <div class="wrap">
                            <ul class="content">
                                <router-link v-for="(item,index) in saveList" :key="index" tag="li"
                                             :to="{name:'detailsAndApproval',params:{billId:item.id,billStatus:'SAVE'}}">
                                    <div class="head">
                                        <div class="left">
                                            <span class="materiel-name">{{item.number}}</span>
                                            <!--<span class="materiel-id">({{controlLength(item.tabNumber,20)}})</span>-->
                            ...
                            <Loading :showloadingwrap="showLoadingWrap2" :showloadingimg="showLoadingImg2"></Loading>
                        </div>
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="list-wrap" ref="wrapper3">
                        <div class="wrap">
                            <ul class="content">
                                <router-link v-for="(item,index) in auditList" :key="index" tag="li"
                                             :to="{name:'detailsAndApproval',params:{billId:item.id,billStatus:'AUDIT'}}">
                                    <div class="head">
                                        <div class="left">
                                            <span class="materiel-name">{{item.number}}</span>
                                            <!--<span class="materiel-id">({{controlLength(item.tabNumber,20)}})</span>-->
                                ...
                                <!--<li v-for="(item,index) in auditList" :key="index">-->
                                <!--<div class="head">-->
                                <!--<div class="left">-->
                                <!--<span class="materiel-name">{{item.materialname}}&nbsp;&nbsp;{{item.model}}</span>-->
                                <!--&lt;!&ndash;<span class="materiel-id">({{controlLength(item.tabNumber,20)}})</span>&ndash;&gt;-->
                                <!--</div>-->
                                <!--&lt;!&ndash;<div class="right">&ndash;&gt;-->
                                <!--&lt;!&ndash;<span>{{item.status === 1 ? '正常':'关闭'}}</span>&ndash;&gt;-->
                                <!--&lt;!&ndash;<i class="normal-icon"></i>&ndash;&gt;-->
                                <!--&lt;!&ndash;</div>&ndash;&gt;-->
                                <!--</div>-->
                                ...
                                <!--&lt;!&ndash;<span class="label">{{controlLength(item.organization,8)}}</span>&ndash;&gt;-->
                                <!--</div>-->
                                <!--<div class="more">-->
                                <!--&lt;!&ndash;<router-link :to="">更多</router-link>&ndash;&gt;-->
                                <!--</div>-->
                                <!--</div>-->
                                <!--</li>-->
                            </ul>
                            <Loading :showloadingwrap="showLoadingWrap3" :showloadingimg="showLoadingImg3"></Loading>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!---->
        <!--</div>-->
        <router-link :to="{name:'add',params:{name,userId,companyId}}" tag="div" class="btn">
            <!--<i class="icon"></i>-->
  ...
  export default {
    ...
    beforeRouteEnter(to, from, next) {
      next(vm => {
        ...
        // if (from.path === '/' || from.path === '/add' || from.name.indexOf('detail') > -1
        ...
        vm.$nextTick(() => {
          ...
          vm.loadData(vm.userId, vm.companyId, 'PersonExpense', 'SUBMIT', 1, 10)
        })
    ...
    methods: {
      ...
      loadData(userId, companyId, billType, billStatus, fromRow, toRow) {
        if (billStatus === 'SUBMIT') {
          ...
          axios.get('http://rest.apizza.net/mock/c16745d80509aa726ad0a1ca0797d75f/getBillListPersonExpenseNew', {
          // axios.get(this.$url, {
            params: {
              path: 'getBillList',
              userId: userId,
                ...
                this.$nextTick(() => {
                  ...
                  if (!this.submitscroll) {
                    ...
                    this.submitscroll.on('pullingUp', () => {
                      ...
                      this.loadData(this.userId, this.companyId, 'PersonExpense', billStatus, fromRow += 10, toRow += 10)
      ...
      currentIndex() {
        if (this.currentIndex === 2) {
          if (this.auditList.length === 0) {
            this.loadData(this.userId, this.companyId, 'PersonExpense', 'AUDIT', 1, 10)
          }
        } else if (this.currentIndex === 1) {
          if (this.saveList.length === 0) {
            this.loadData(this.userId, this.companyId, 'PersonExpense', 'SAVE', 1, 10)
          }
        }
      }
    }
