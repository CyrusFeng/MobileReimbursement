<template>
    <div style="height: 100%;">
        <GoBack v-if="$store.state.UA !== 1">出差申请单列表</GoBack>
                        ...
                        <div class="wrap">
                            <ul class="content">
                                <router-link
                                        ...
                                        :key="index"
                                        :to="{name:'detail',params:{billId:item.id,billStatus:'SUBMIT'}}"
                                >
                                    <div class="head">
                                        <div class="left">
                        ...
                        <div class="wrap">
                            <ul class="content">
                                <router-link v-for="(item,index) in saveList" :key="index" tag="li"
                                             :to="{name:'detail',params:{billId:item.id,billStatus:'SAVE'}}">
                                    <div class="head">
                                        <div class="left">
                                            <span class="materiel-name">{{item.number}}</span>
                                            <!--<span class="materiel-id">({{controlLength(item.tabNumber,20)}})</span>-->
                        ...
                        <div class="wrap">
                            <ul class="content">
                                <router-link v-for="(item,index) in auditList" :key="index" tag="li"
                                             :to="{name:'detail',params:{billId:item.id,billStatus:'AUDIT'}}">
                                    <div class="head">
                                        <div class="left">
                                            <span class="materiel-name">{{item.number}}</span>
                                            <!--<span class="materiel-id">({{controlLength(item.tabNumber,20)}})</span>-->
        ...
        <!---->
        <!--</div>-->
        <router-link :to="{name:'add',params:{name:'TravelApply',userId,companyId}}" tag="div" class="btn">
            <!--<i class="icon"></i>-->
    ...
    beforeRouteEnter(to, from, next) {
      next(vm => {
        ...
        vm.$nextTick(() => {
          ...
          vm.loadData(vm.userId, vm.companyId, 'TravelApply', 'SUBMIT', 1, 10)
        })
    ...
    methods: {
      ...
      loadData(userId, companyId, billType, billStatus, fromRow, toRow) {
        ...
        if (billStatus === 'SUBMIT') {
          axios.get('http://rest.apizza.net/mock/c16745d80509aa726ad0a1ca0797d75f/getBillListTravelApplyNew', {
          // axios.get(this.$url, {
            params: {
              path: 'getBillList',
              userId: userId,
